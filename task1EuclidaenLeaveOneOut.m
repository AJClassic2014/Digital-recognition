%****classification by Euclidean distance(Leave-One-Out)********
...
if temp~=2    
    ...
    while sample<=100%100 samples for each pattern 
       ...
       while member<=100
        ...
        meanlist{1,temp}=newmean;%put the new mean value into the mean list
        min=inf;
        total=cell(1,8);
        distance=1;
...
if temp==2%for numeric pattern "2"(57 samples)
       sample=1;
...
fprintf('********percentage of correct for classification by Euclidean(Leave-One-Out)**************************\n');
totalRow=2;
while totalRow<=9
