...
while temp<=length(classlist)%get every one from 8 numeric class
    ...
    while sample<=length(classlist{temp,1})%100 samples for each pattern 
        ...
        getmin=1;
        while getmin<=3%sort list 3 times and get 3 smallest one
        minvalue=inf;
        comparenum=1;
...
fprintf('********percentage of correct for classification by 3-NN**************************\n');
totalRow=2;
while totalRow<=9
